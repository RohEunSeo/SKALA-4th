...
import com.skala.stock.dto.StockPriceUpdateDto;
...
public class StockController {
    ...
    @Operation(summary = "주식 가격 변경", description = "주식의 현재가를 변경합니다 (기존 현재가는 전일 종가로 이동합니다)")
    public ResponseEntity<StockDto> updatePrice(
            @PathVariable Long id,
            @Valid @RequestBody StockPriceUpdateDto priceUpdateDto) {
        StockDto stock = stockService.updatePrice(id, priceUpdateDto.getCurrentPrice());
        return ResponseEntity.ok(stock);
    }

    @DeleteMapping("/{id}")
    @Operation(summary = "주식 삭제", description = "ID로 주식을 삭제합니다 (보유/거래 이력이 있으면 삭제할 수 없습니다)")
