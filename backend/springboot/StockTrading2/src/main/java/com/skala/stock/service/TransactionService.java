    ...
    public TransactionDto executeTrade(TradeRequestDto tradeRequest) {
        ...
        if (tradeRequest.getType() == Transaction.TransactionType.BUY) {
            ...
            if (user.getBalance() < totalAmount) {
                String message = "잔액이 부족합니다. 필요 금액: " + totalAmount + ", 보유 금액: " + user.getBalance();
                tradeAuditService.recordWarning(user.getId(), stock.getId(), tradeRequest.getType(), message);
            ...
            if (portfolio == null || portfolio.getQuantity() < tradeRequest.getQuantity()) {
                String message = "보유 수량이 부족합니다. 보유 수량: " + (portfolio != null ? portfolio.getQuantity() : 0) + ", 매도 수량: " + tradeRequest.getQuantity();
                tradeAuditService.recordWarning(user.getId(), stock.getId(), tradeRequest.getType(), message);
                throw new RuntimeException(message);
            }
            // 잔액 증가
